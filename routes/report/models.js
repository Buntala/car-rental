async function getCompanyIncomeReport(psql,data){
    ...
    let query = `
        ...
        where b.canceled=false and b.finished = true ${data.start_time?`and start_time >= '${data.start_time}'`:''} ${data.end_time?`and start_time <= '${data.end_time}'`:''}
    `
...
async function getDriverBookingReport(psql,data){
    ...
    coalesce(sum(b.driver_incentive),0) as "total_incentive"
    from booking b 
    where b.canceled=false and b.finished = true and b.driver_id ${data.id?'='+data.id:' is not null'}  ${data.start_time?`and start_time >= '${data.start_time}'`:''} ${data.end_time?`and start_time <= '${data.end_time}'`:''}
...
async function getCarBookingReport(psql,data){
    ...
    from booking b
    where b.canceled=false and b.finished = true and b.cars_id${data.id?'='+data.id:' is not null'}  ${data.start_time?`and start_time >= '${data.start_time}'`:''} ${data.end_time?`and start_time  <= '${data.end_time}'`:''}
    group by b.cars_id
    order by b.cars_id;`
    let result = await psql.query(query);
    return result.rows;
