//query functions
const getMembershipData = require('../membership/models').getMembershipData;
const getDriverData = require('../driver/models').getDriverData;
const getCarData = require('../car/models').getCarData;
const getCustData = require('../customer/models').getCustData;
...
async function insertBookingData(psql, data){
    ...
    let notAvailable = await availabilityCheck(psql,data.car_id,data.driver_id,data.start_time,data.end_time);
    if (notAvailable){
...
async function updateBookingData(psql,data){
    ...
    let notAvailable = await availabilityCheck(psql,data.car_id,data.driver_id,data.start_time,data.end_time,current_data.cars_id,current_data.driver_id);
    if (notAvailable){
        throw new Error(notAvailable)
    }
    const {
...
async function calculateAll(psql,data){
    ...
    console.log('member: '+membership_discount)
...
async function getMembershipDiscount(psql,id){
    try{
        ...
        if (cust_data.membership_id){
            ...
            let result = await getMembershipData(psql,data);
...
async function driverAvailability(psql, driver_id,start_time,end_time){
    query = 
    `
    SELECT * FROM booking
     WHERE (finished='false' AND driver_id = ${driver_id} )AND
    (
...
async function availabilityCheck(psql,cars_id,driver_id,start_time,end_time,current_car=null,current_driver=null){
    ...
    if (!same_driver && driver_id){
        var driverStatus = await driverAvailability(psql,driver_id,start_time,end_time);
    }
